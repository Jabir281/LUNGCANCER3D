"""
3D Hybrid CNN with MLP Head for Lung Nodule Classification
===========================================================
Combines three parallel 3D CNN feature extractors whose outputs are
concatenated and fused by a shared MLP classification head.
...
# ── NOTE: batch size is reduced from 8 → 4.
#    Three large 3D CNNs run in parallel, roughly tripling peak GPU memory.
#    Lower to 2 if you still encounter OOM errors.
BATCH_SIZE         = 8
NUM_WORKERS        = 4
PIN_MEMORY         = True
...
class HybridCNNWithMLPHead(nn.Module):
    """
    3D Hybrid CNN: three parallel feature extractors + MLP fusion head.
...
def main():
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if val_auc > best_val_auc:
            ...
            with open("best_metrics_hybrid.json", "w") as f:
                json.dump({
                    "epoch":       epoch,
                    "auc":         val_auc,
                    "auprc":       val_auprc,
    ...
    pd.DataFrame(history).to_csv("hybrid_training_log.csv", index=False)
    print("\n[Saved] hybrid_training_log.csv")
    ...
    model.load_state_dict(
        torch.load("best_model_hybrid.pth", map_location=device, weights_only=True)
    )
