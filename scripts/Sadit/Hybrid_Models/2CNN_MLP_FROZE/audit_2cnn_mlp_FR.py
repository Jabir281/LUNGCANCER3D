# audit_hybrid2_frozen.py
"""
Full audit for 2‑Branch Hybrid (ResNet18 + DenseNet121) with frozen backbones and MLP head.
...
MODEL_PATH = "best_model_hybrid2_frozen.pth"
PREDICTIONS_PATH = "hybrid2_frozen_test_predictions.csv"
TRAINING_LOG_PATH = "hybrid2_frozen_training_log.csv"
BEST_METRICS_PATH = "best_metrics_hybrid2_frozen.json"
...
# ======================== Model (same as training, but without weight loading) ========
...
# ======================== Dataset (exact replica of training, no transforms) ========
...
class AuditReport:
    ...
    def save(self, txt_path, json_path):
        lines = ["="*70, "  HYBRID2 (FROZEN) MLP AUDIT REPORT", "="*70,
...
def check_2_weights_trained(report, device):
    ...
    try:
        ...
        trained.freeze_backbones()    # ensure backbones are frozen for the check

        random_model = HybridTwoBranch().to(device)
        # Compare first MLP head weight (Linear 1536->512)
        ...
        if rn_frozen and dn_frozen:
            report.print_check("Backbones frozen", "PASS", "All backbone parameters have requires_grad=False")
        else:
            report.print_check("Backbones frozen", "FAIL", f"ResNet18: {rn_frozen}, DenseNet121: {dn_frozen}")
        return trained
...
def check_3_predictions_csv(report):
    ...
    try:
        ...
        report.print_check("Load CSV", "FAIL", str(e))
        return None
...
def main():
    ...
    print("="*70)
    print("  AUDIT: 2‑Branch Hybrid (Frozen) + MLP Head")
    print("="*70)
    ...
    print("="*70)
    report.save("audit_report_hybrid2_frozen.txt", "audit_summary_hybrid2_frozen.json")
