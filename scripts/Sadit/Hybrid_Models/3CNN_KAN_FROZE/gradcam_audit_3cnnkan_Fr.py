# gradcam_hybrid2_kan_frozen_full_eval_audit.py
"""
Full evaluation + GradCAM++ + Audit for the 2‑Branch Hybrid
(ResNet18 + DenseNet121, frozen backbones, KAN head).

Loads the trained model (best_model_hybrid2_kan.pth), runs test inference,
computes all metrics, saves predictions & calibration, generates GradCAM++
overlays for selected TP/FN/FP/TN cases, and performs a 9‑point audit.
"""
...
MODEL_PATH = "best_model_hybrid2_kan.pth"                     # trained model
PREDICTIONS_PATH = "hybrid2_kan_test_predictions.csv"         # from training
...
# KAN hyperparameters (must match training)
KAN_GRID_SIZE = 5
KAN_SPLINE_ORDER = 3
KAN_SCALE_NOISE = 0.1
KAN_SCALE_BASE = 1.0
...
class KANLinear(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        with torch.no_grad():
            ...
            noise = (torch.rand(inner.size(0), self.in_features, self.out_features) - 0.5
    ...
    def _b_splines(self, x):
        ...
        for k in range(1, self.spline_order + 1):
            ...
            left_w = torch.where(left_denom > 1e-8,
                                 (xv - g[:, :-(k+1)]) / left_denom.clamp(min=1e-8),
                                 torch.zeros_like(xv))
            right_w = torch.where(right_denom > 1e-8,
    ...
    def update_grid(self, x, margin=0.01):
        # Not needed for evaluation; placeholder to satisfy KAN interface
        pass
...
# ----------------------------- Dataset (identical to training, with filepath) -
...
class AuditReport:
    ...
    def save(self, txt_path, json_path):
        lines = ["="*70, "  HYBRID2 + KAN (FROZEN) AUDIT REPORT", "="*70,
...
def audit_4_recompute(report, df):
    print("\n[CHECK 4] Recompute Metrics from Predictions")
...
def audit_6_live(report, model, device):
    print("\n[CHECK 6] Live Inference vs Saved Predictions")
...
def main():
    ...
    for case_type, num, row in selected:
        ...
        orig, hm, ov = produce_heatmap(model, input_t, target_layer_name, class_idx=cidx)
    ...
    print("\n"+"="*70+"\n  STARTING AUDIT\n"+"="*70)
    report = AuditReport()
    # Ensure the predictions CSV expected by audit exists (copy from ours if needed)
    if not os.path.exists(PREDICTIONS_PATH):
        pred_df.to_csv(PREDICTIONS_PATH, index=False)
    files_ok = audit_1_files(report)
    model_audit = None
    if files_ok:
        model_audit = audit_2_weights(report, DEVICE)
    df = audit_3_csv(report)
    if df is not None:
        audit_4_recompute(report, df)
    if os.path.exists(TRAINING_LOG_PATH):
        audit_5_log(report)
    audit_6_live(report, model if model is not None else model_audit, DEVICE)
