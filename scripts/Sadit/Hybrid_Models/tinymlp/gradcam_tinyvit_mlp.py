# gradcam_hybrid_resnet_tinyvit_full_eval.py
"""
Full evaluation + GradCAM++ for Hybrid ResNet18 (3D) + TinyViT (2D).
...
MODEL_PATH = "best_model_hybrid_resnet_tinyvit.pth"   # from training
BATCH_SIZE = 1                # GradCAM with batch 1
NUM_WORKERS = 0
...
class HybridDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 3D branch: (1,64,64,64)
        patch_3d = np.expand_dims(vol, axis=0)
        # 2D branch: central 3 slices → (3,64,64)
        centre = vol.shape[0] // 2
        slices = [vol[centre-1, :, :], vol[centre, :, :], vol[centre+1, :, :]]
        img_2d = np.stack(slices, axis=0)

        # Apply transforms
        if self.transform_3d is not None:
            # For evaluation, transform_3d is Compose([]) – identity.
            # It expects an array input and returns a tensor.
            patch_3d = self.transform_3d(patch_3d)
        ...
        return (patch_3d, img_2d), torch.tensor(label, dtype=torch.float32), row["seriesuid"], row["filepath"]
...
class HybridResNetTinyViT(nn.Module):
    def __init__(self, num_classes=1, dropout=0.5):
        ...
        # determine dims (same as training)
        with torch.no_grad():
            dummy_vol = torch.zeros(1, 1, 64, 64, 64)
            dummy_img = torch.zeros(1, 3, 224, 224)
            f3 = self.resnet(dummy_vol)
            f2 = self.tinyvit(dummy_img)
        dim3 = f3.shape[1]
        dim2 = f2.shape[1]
        self.mlp_head = nn.Sequential(
            nn.Linear(dim3 + dim2, 256),
            nn.LayerNorm(256),
    ...
    def forward(self, vol_3d, img_2d):
        f3 = self.resnet(vol_3d)
        f2 = self.tinyvit(img_2d)
        combined = torch.cat([f3, f2], dim=1)
...
def find_target_layer(model):
    """
    Locate the target convolutional layer in the ResNet18 branch.
    We use 'resnet.layer3.1' (second BasicBlock of layer3), same as standalone ResNet.
    """
    for name, mod in model.named_modules():
        if name == 'resnet.layer3.1':
            print(f"✔ Chosen target layer: {name}")
            return name
    raise ValueError("Target layer 'resnet.layer3.1' not found in model.")
...
def produce_heatmap(model, input_3d, input_2d, target_layer_name, class_idx=1):
    """
    Generate GradCAM++ heatmap for the 3D branch.
    Both inputs are required; the 2D branch is passed through but gradients
    are only used for the ResNet18 branch.
    """
    model.eval()
    features = {}
    gradients = {}
    ...
    # Ensure inputs require grad only on the 3D branch (the 2D input can be detached or not, but gradients will flow)
    # Actually, we just need the full forward with both inputs.
    logits = model(input_3d, input_2d)          # single scalar
    ...
    # Central depth slice from heatmap
    ...
    vol = input_3d.squeeze(0).squeeze(0).cpu().numpy()   # (64,64,64)
...
def main():
    ...
    # Transforms: 3D identity, 2D resize
    eval_3d = Compose([])          # identity
    eval_2d = Compose([Resized(keys=["img"], spatial_size=RESIZE_TO, mode="bicubic")])

    test_dataset = HybridDataset(test_meta, DATA_DIR, transform_3d=eval_3d, transform_2d=eval_2d)
    ...
    with torch.no_grad():
        for (patch_3d, img_2d), labels, uids, fpaths in tqdm(test_loader, desc="Predicting"):
            patch_3d = patch_3d.to(DEVICE)
    ...
    pred_df.to_csv("hybrid_resnet_tinyvit_gradcam_test_predictions.csv", index=False)
    print("[Saved] Test predictions exported to 'hybrid_resnet_tinyvit_gradcam_test_predictions.csv'")

    cal_df = pd.DataFrame({
        'mean_predicted_probability': mean_predicted_value,
        'fraction_of_positives': fraction_of_positives
    })
    cal_df.to_csv("hybrid_resnet_tinyvit_gradcam_calibration_curve.csv", index=False)
    print("[Saved] Calibration curve exported to 'hybrid_resnet_tinyvit_gradcam_calibration_curve.csv'")
    ...
    for case_type, case_num, row in selected_cases:
        uid = row['seriesuid']
        # Find best candidate for this patient
        ...
        (patch_3d, img_2d), label, uid_out, _ = test_dataset[idx_in_meta]

        input_3d = patch_3d.unsqueeze(0).to(DEVICE)   # (1,1,64,64,64)
        input_2d = img_2d.unsqueeze(0).to(DEVICE)     # (1,3,224,224)
        ...
        orig_slice, hm_slice, overlay = produce_heatmap(
            model, input_3d, input_2d, target_layer_name, class_idx=cidx
        )
    ...
    plt.savefig("gradcampp_hybrid_resnet_tinyvit_analysis.png", dpi=300, bbox_inches='tight')
    plt.show()
    print("\nSaved: gradcampp_hybrid_resnet_tinyvit_analysis.png")
