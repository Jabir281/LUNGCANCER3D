# gradcam_hybrid2_mlp_full_eval.py
"""
Full evaluation + GradCAM++ for 2-Branch Hybrid (ResNet18 + DenseNet121) with MLP head.
Computes all test metrics, saves predictions, calibration curve,
and generates GradCAM++ analysis image.
Target layer: ResNet18 branch's layer3.1 (second BasicBlock of layer3).
"""
...
MODEL_PATH = "best_model_hybrid2.pth"          # from training script (MLP version)
BATCH_SIZE = 1
NUM_WORKERS = 0
...
def main():
    ...
    pred_df.to_csv("hybrid2_mlp_gradcam_test_predictions.csv", index=False)
    print("[Saved] Test predictions exported to 'hybrid2_mlp_gradcam_test_predictions.csv'")

    cal_df = pd.DataFrame({
        'mean_predicted_probability': mean_predicted_value,
        'fraction_of_positives': fraction_of_positives
    })
    cal_df.to_csv("hybrid2_mlp_gradcam_calibration_curve.csv", index=False)
    print("[Saved] Calibration curve exported to 'hybrid2_mlp_gradcam_calibration_curve.csv'")
    ...
    plt.savefig("gradcampp_hybrid2_mlp_analysis.png", dpi=300, bbox_inches='tight')
    plt.show()
    print("\nSaved: gradcampp_hybrid2_mlp_analysis.png")
