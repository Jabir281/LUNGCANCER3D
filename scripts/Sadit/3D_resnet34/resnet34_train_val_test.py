"""
3D ResNet34 with MLP Head for Lung Nodule Classification
...
from monai.networks.nets.resnet import ResNetBottleneck, ResNetBlock
...
# ResNet34 feature dimension — fixed architecture property
# ResNet34 uses BasicBlock, final layer outputs 512 channels
...
class ResNet34WithMLPHead(nn.Module):
    """
    3D ResNet34 backbone from MONAI with a custom MLP classification head.

    ResNet34 uses BasicBlock (not Bottleneck) with layer config [3, 4, 6, 3].
    Final feature dimension before the classifier is 512.

    The original FC head is replaced with:
        Linear(512 → 256) → LayerNorm → ReLU → Dropout → Linear(256 → 1)

    This mirrors the DenseNet121 MLP head structure exactly,
    only the input dimension changes (512 instead of 1024).
    """
    def __init__(self, in_channels=1, num_classes=1, dropout=0.5):
        ...
        # layers=[3,4,6,3]  → ResNet34 depth
        # block_inplanes defines channel widths per stage: [64,128,256,512]
        self.backbone = ResNet(
            ...
            num_classes=RESNET34_FEATURE_DIM,  # Trick: set num_classes=512
        ...
        # Input: 512-d (ResNet34 penultimate features after global avg pool)
...
def evaluate_model(loader, model, device, desc="Validating", return_preds=False):
    ...
    if return_preds:
        ...
        cal_df.to_csv("resnet34_calibration_curve.csv", index=False)
        print("[Saved] Calibration data exported to 'resnet34_calibration_curve.csv'")
...
def main():
    ...
    model = ResNet34WithMLPHead().to(device)

    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print("\n--- Model Summary ---")
    print(f"Model         : 3D ResNet34 + MLP Head")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if val_auc > best_val_auc:
            ...
            with open("best_metrics_resnet34.json", "w") as f:
    ...
    pd.DataFrame(history).to_csv("resnet34_training_log.csv", index=False)
    print("\n[Saved] Training history exported to 'resnet34_training_log.csv'")
    ...
    pred_df.to_csv("resnet34_test_predictions.csv", index=False)
    print("[Saved] Test predictions exported to 'resnet34_test_predictions.csv'")
