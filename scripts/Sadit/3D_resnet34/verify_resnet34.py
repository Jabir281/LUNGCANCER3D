...
ResNet34 - Lung Nodule Classification (LUNA16)
=============================================================================

PURPOSE:
    Independently verifies that ResNet34 model results are legitimate by running
    multiple sanity checks across the data, predictions, model weights,
    and statistical properties. Generates a full audit report.

HOW TO RUN:
    python verify_resnet34.py

PREREQUISITES (files must exist from training):
    - best_model_resnet34.pth
    - resnet34_test_predictions.csv
    - resnet34_training_log.csv
    - best_metrics_resnet34.json
    - metadata_all.csv  (or patient_split.csv)

OUTPUT:
    - audit_report_resnet34.txt   (human-readable full report)
    - audit_summary_resnet34.json (machine-readable pass/fail summary)
=============================================================================
"""
...
# =============================================================================
# CONFIGURATION — Match exactly with your ResNet34 training script
# =============================================================================
SEED = 42
DATA_DIR = r"C:\Users\T2520789\LUNGCANCER3D\data"   # <-- adjust if different
METADATA_PATH = os.path.join(DATA_DIR, "metadata_all.csv")
PATIENT_SPLIT_PATH = os.path.join(DATA_DIR, "patient_split.csv")
MODEL_PATH = "best_model_resnet34.pth"
PREDICTIONS_PATH = "resnet34_test_predictions.csv"
TRAINING_LOG_PATH = "resnet34_training_log.csv"
BEST_METRICS_PATH = "best_metrics_resnet34.json"
BATCH_SIZE = 8
NUM_WORKERS = 4
...
# =============================================================================
# MODEL (must match ResNet34 training script exactly)
# =============================================================================
class ResNet34WithMLPHead(nn.Module):
    def __init__(self, in_channels=1, num_classes=1, dropout=0.5):
        ...
        # Replace final FC layer with MLP head
...
class AuditReport:
    ...
    def save(self, txt_path, json_path):
        lines = []
        lines.append("=" * 70)
        lines.append("  MODEL AUDIT REPORT — ResNet34 Lung Nodule Classification")
        lines.append("=" * 70)
...
# =============================================================================
# AUDIT CHECKS (identical logic, only file paths differ)
# =============================================================================
...
def check_2_model_weights_not_random(report, device):
    ...
    try:
        ...
        mlp_w = list(trained_model.backbone.fc.parameters())[0].detach().cpu().numpy()
        mlp_std = mlp_w.std()
        if mlp_std < 1e-6:
...
def main():
    ...
    print("=" * 70)
    print("  MODEL RESULTS AUDIT — ResNet34 Lung Nodule Classification")
    ...
    report.save("audit_report_resnet34.txt", "audit_summary_resnet34.json")
