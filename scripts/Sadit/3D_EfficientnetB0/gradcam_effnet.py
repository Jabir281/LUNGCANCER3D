# gradcam_effnetb0_full_eval.py
"""
Full evaluation + GradCAM++ for 3D EfficientNet-B0.
Computes all test metrics, saves predictions, calibration curve,
and generates GradCAM++ analysis image.

Model architecture: 3D EfficientNet-B0 backbone + MLP head.
Target layer for GradCAM++: backbone.head_conv (final conv layer before global pooling).
"""
...
# ----------------------------- 3D EfficientNet-B0 (from training script) -----------------
...
class EfficientNet3D_B0(nn.Module):
    def __init__(self, in_channels=1):
        ...
        self.head_conv = nn.Sequential(
            nn.Conv3d(320, 1280, kernel_size=1, bias=False),
    ...
    def forward(self, x):
        x = self.stem(x)
        x = self.stages(x)
        x = self.head_conv(x)   # final conv layer we target
...
def find_target_layer(model):
    """
    Returns the name of the target layer for EfficientNet-B0.
    We choose the final convolutional layer: 'head_conv' (before global pooling).
    """
    for name, mod in model.named_modules():
        # 'backbone.head_conv' is the whole Sequential; we want to attach hooks to it.
        if 'head_conv' in name and isinstance(mod, nn.Sequential):
            print(f"✔ Chosen target layer (head_conv): {name}")
            return name
    # Fallback: output of last MBConv stage
    for name, mod in model.named_modules():
        if 'stages' in name and isinstance(mod, nn.Sequential):
            print(f"✔ Fallback target layer (stages): {name}")
            return name
    raise ValueError("Could not find a suitable target layer in model.")
...
def produce_heatmap(model, input_tensor, target_layer_name, class_idx=1):
    ...
    # Extract central slice in depth dimension
...
def main():
    ...
    pred_df.to_csv("efficientnetb0_gradcam_test_predictions.csv", index=False)
    print("[Saved] Test predictions exported to 'efficientnetb0_gradcam_test_predictions.csv'")

    cal_df = pd.DataFrame({
        'mean_predicted_probability': mean_predicted_value,
        'fraction_of_positives': fraction_of_positives
    })
    cal_df.to_csv("efficientnetb0_gradcam_calibration_curve.csv", index=False)
    print("[Saved] Calibration curve exported to 'efficientnetb0_gradcam_calibration_curve.csv'")
    ...
    plt.savefig("gradcampp_efficientnetb0_analysis.png", dpi=300, bbox_inches='tight')
    plt.show()
    print("\nSaved: gradcampp_efficientnetb0_analysis.png")
