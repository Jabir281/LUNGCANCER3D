# gradcam_resnet18_full_eval.py
"""
Full evaluation + GradCAM++ for 3D ResNet18.
Computes all test metrics, saves predictions, calibration curve,
and generates GradCAM++ analysis image.

Model architecture: MONAI ResNet18 (3D) + MLP head.
Target layer for GradCAM++: backbone.layer3.1 (or backbone.layer4 as fallback).
"""
...
class ResNet18WithMLPHead(nn.Module):
    def __init__(self):
        ...
        # Replace the final fc with MLP head (same as DenseNet/EfficientNet scripts)
...
def find_target_layer(model):
    """
    Locate a suitable convolutional layer for GradCAM++.
    For ResNet18, we target the second block of layer3.
    """
    target_name = None
    for name, mod in model.named_modules():
        if 'backbone.layer3.1' in name:
            target_name = name
            break
    if target_name is None:
        # fallback to layer4 output
        for name, mod in model.named_modules():
            if 'backbone.layer4' in name and 'conv' in name:
                target_name = name
                break
    if target_name:
        print(f"✔ Chosen target layer: {target_name}")
        return target_name
    else:
        raise ValueError("Could not find a suitable target layer in ResNet18.")
...
def main():
    ...
    pred_df.to_csv("resnet18_gradcam_test_predictions.csv", index=False)
    print("[Saved] Test predictions exported to 'resnet18_gradcam_test_predictions.csv'")

    cal_df = pd.DataFrame({
        'mean_predicted_probability': mean_predicted_value,
        'fraction_of_positives': fraction_of_positives
    })
    cal_df.to_csv("resnet18_gradcam_calibration_curve.csv", index=False)
    print("[Saved] Calibration curve exported to 'resnet18_gradcam_calibration_curve.csv'")
    ...
    plt.savefig("gradcampp_resnet18_analysis.png", dpi=300, bbox_inches='tight')
    plt.show()
    print("\nSaved: gradcampp_resnet18_analysis.png")
