"""
3D DenseNet121 with MLP Head for Lung Nodule Classification
Includes LayerNorm, Robust Metrics, Model Checkpoints, and Prediction Tracking.
"""
...
from sklearn.metrics import roc_auc_score, average_precision_score, f1_score, confusion_matrix, roc_curve, recall_score
...
class DenseNet121WithMLPHead(nn.Module):
    def __init__(self, in_channels=1, num_classes=1, dropout=0.5):
        super().__init__()
        # CHANGE 3: out_channels=1 for cleaner construction before overriding
        self.backbone = DenseNet121(spatial_dims=3, in_channels=in_channels, out_channels=1)
        
        assert hasattr(self.backbone, 'class_layers'), "MONAI DenseNet structure changed: 'class_layers' missing"
        assert hasattr(self.backbone.class_layers, 'out'), "MONAI DenseNet structure changed: 'out' missing"

        self.backbone.class_layers.out = nn.Sequential(
            nn.Linear(1024, 256),
            nn.LayerNorm(256),     # CHANGE 1: Replaced BatchNorm with LayerNorm
...
def calculate_candidate_froc(all_labels, all_probs, total_scans):
    """
    CRITICAL FIX: Candidate-level FROC.
    Evaluates every single patch, not just the max per patient.
    """
    all_labels = np.array(all_labels)
    all_probs = np.array(all_probs)
    
    total_positives = (all_labels == 1).sum() # Total true nodules
    
    # Sort predictions by probability descending
    descending_indices = np.argsort(all_probs)[::-1]
    sorted_probs = all_probs[descending_indices]
    ...
    for target in FROC_THRESHOLDS:
        # Find the index where fps_per_scan is closest to, but not exceeding, the target
        valid_idx = np.where(fps_per_scan <= target)[0]
        if len(valid_idx) > 0:
            sens = sensitivity[valid_idx[-1]]
        else:
            sens = 0.0
...
def calculate_95_ci(y_true, y_probs, n_bootstraps=1000):
    """Calculates 95% Confidence Interval for AUROC using Bootstrapping."""
    bootstrapped_scores = []
    ...
    for i in range(n_bootstraps):
        ...
        if len(np.unique(np.array(y_true)[indices])) < 2:
            continue # Skip if bootstrap sample only has one class
...
def evaluate_model(loader, model, device, desc="Validating", return_preds=False):
    ...
    # 1. Patient-Level Aggregation (For standard AUROC / Diagnosis)
    patient_dict = {}
    ...
    # 2. Candidate-Level Metrics (CRITICAL FOR FROC)
    froc = calculate_candidate_froc(all_labels, all_probs, total_scans)
    
    if return_preds:
        # Confidence Intervals for final test reporting
        ...
        fraction_of_positives, mean_predicted_value = calibration_curve(y_true_patient, y_probs_patient, n_bins=10)
        cal_df = pd.DataFrame({
            'mean_predicted_probability': mean_predicted_value,
            'fraction_of_positives': fraction_of_positives
        })
        cal_df.to_csv("densenet121_calibration_curve.csv", index=False)
        print("[Saved] Calibration data exported to 'densenet121_calibration_curve.csv'")
        
        pred_df = pd.DataFrame({
            'seriesuid': all_uids,      # Saving candidate-level predictions
            'label': all_labels,
...
def main():
    ...
    best_val_auc = 0.0
    patience_counter = 0
    history = [] # CHANGE 4: Training History Tracker
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        # Validation
        val_loss, val_auc, val_auprc, val_f1, val_sens, val_spec, val_froc = evaluate_model(val_loader, model, device, desc="Validating")
        
        # CHANGE 4: Append to History
        history.append({
            "epoch": epoch,
            "lr": current_lr,
        ...
        if val_auc > best_val_auc:
            ...
            # CHANGE 5: Save Best Metrics to JSON
            best_metrics = {
                "epoch": epoch,
                "auc": val_auc,
                "auprc": val_auprc,
            ...
            with open("best_metrics_densenet121.json", "w") as f:
    ...
    # CHANGE 4: Save History to CSV
    pd.DataFrame(history).to_csv("densenet121_training_log.csv", index=False)
    print("\n[Saved] Training history exported to 'densenet121_training_log.csv'")
    ...
    # CHANGE 7: Load Safely with map_location
    model.load_state_dict(torch.load("best_model_densenet121.pth", map_location=device, weights_only=True))
    
    # CHANGE 8: Added return_preds=True to get the dataframe
    ...
    pred_df.to_csv("densenet121_test_predictions.csv", index=False)
    print("[Saved] Test predictions exported to 'densenet121_test_predictions.csv'")
