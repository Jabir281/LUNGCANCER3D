...
# ============ DATASET (same as your training script) ============
...
# ============ MODEL DEFINITIONS (must match your training script) ============
...
def evaluate_metrics(probs, labels, uids):
    ...
    # FIX: Only use achievable FP rates
...
def main():
    ...
    for name, (model, ckpt_path) in models_to_eval.items():
        ...
        print(f"  CPM:                  {metrics['CPM']:.4f}")
        print(f"  Achievable FROC rates: {metrics['FROC_rates']}")
        for r, s in zip(metrics['FROC_rates'], metrics['FROC_sensitivities']):
            print(f"    FP/scan: {r} -> Sensitivity: {s:.4f}")
    
    # Save to markdown log
    log_path = Path("re_evaluate_results.md")
    with open(log_path, "w") as f:
        f.write("# FIXED CPM RE-EVALUATION\n\n")
        f.write("| Model | Sensitivity (Recall) | Specificity | ROC-AUC | F1-Score | CPM |\n")
        f.write("| --- | --- | --- | --- | --- | --- |\n")
        
        for name, metrics in results.items():
            f.write(f"| {name} | {metrics['Sensitivity (Recall)']:.4f} | {metrics['Specificity']:.4f} | {metrics['ROC-AUC']:.4f} | {metrics['F1-Score']:.4f} | {metrics['CPM']:.4f} |\n")
            
        f.write("\n## Achievable FROC Rates\n\n")
        for name, metrics in results.items():
            f.write(f"### {name}\n")
            for r, s in zip(metrics['FROC_rates'], metrics['FROC_sensitivities']):
    ...
    print(f"\n{'='*50}")
    print(f"All results saved to markdown: {log_path.absolute()}")
