...
# Import your new ResNet-50 architecture (UPDATED)
...
def main():
    ...
    # Dataset Directories (Make sure these exist relative to where you run the script)
    train_dir = Path("../data/train") if not Path("data/train").exists() else Path("data/train")
    val_dir = Path("../data/val") if not Path("data/val").exists() else Path("data/val")
    
    train_dataset = LungCancerDatasetWithUID(train_dir, transform=train_transforms)
    val_dataset = LungCancerDatasetWithUID(val_dir)
    
    train_loader = DataLoader(train_dataset, batch_size=8, shuffle=True, pin_memory=True, drop_last=True)
    val_loader = DataLoader(val_dataset, batch_size=8, shuffle=False, pin_memory=True)
        
    # Initialize the ResNet50_MLP Model (UPDATED)
    model = ResNet50_MLP(pretrained=False).to(device)
    ...
    for epoch in range(epochs):
        ...
        if current_auc > best_auc:
            ...
            # Changed save string to match your new architecture (UPDATED)
            torch.save(model.state_dict(), "ResNet50_MLP_best.pth")
