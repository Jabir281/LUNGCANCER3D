...
OUTPUT:
    - audit_report_densenet121.txt   (human-readable full report)
    - audit_summary_densenet121.json (machine-readable pass/fail summary)
=============================================================================
"""
...
def check_1_required_files_exist(report):
    """Verify all required output files from training actually exist."""
...
def check_2_model_weights_not_random(report, device):
    """
    Load saved weights and compare against a fresh random initialization.
    If they're identical, the model was never actually trained.
    """
    ...
    try:
        ...
        correlation, p_value = stats.pearsonr(trained_w[:1000], random_w[:1000])
...
def check_3_predictions_csv_integrity(report):
    """
    Validate the saved predictions CSV for internal consistency:
...
def check_4_recompute_metrics_from_predictions(report, pred_df, saved_metrics):
    """
    Recompute AUROC, AUPRC, F1, Sensitivity, Specificity from the saved
    prediction CSV and compare against the saved best_metrics JSON.
    They must match within tolerance — if they don't, metrics were fabricated.
    """
    print("\n[CHECK 4] Recompute Metrics from Saved Predictions")


    # Patient-level aggregation (same logic as training script)
    patient_dict = {}
    ...
    # Note: saved_metrics come from validation (best val epoch), not test
    # So we only verify internal consistency of the prediction CSV here
    ...
    if recomputed["sensitivity"] == 0.0:
        report.print_check("Sensitivity > 0", "FAIL",
                            "Sensitivity=0.0 — model never predicted a single positive. Check threshold.")
...
def check_5_training_log_shows_learning(report):
    """
    Verify the training log shows genuine learning:
    - Loss decreases over time
    - AUROC increases over time
    - No suspiciously flat curves
    """
    ...
    try:
        ...
        if last_loss < first_loss:
            ...
            report.print_check("Training loss decreased", "FAIL",
                                f"Loss did NOT decrease: {first_loss:.4f} → {last_loss:.4f}. Training may have diverged.")


        # Val AUROC should generally increase
        ...
        # Check for suspiciously flat loss (memorization / frozen weights)
        ...
        # Check val AUC is not suspiciously perfect every epoch
...
def check_6_model_inference_live(report, model, device):
    """
    Load actual test patches from disk and run live inference.
    Compare live predictions against saved CSV.
    This is the strongest check — fake results cannot survive this.
    """
    ...
    try:
        ...
        # Sample 50 random patches for spot-check (mix of pos/neg)
        ...
        if checked == 0:
            report.print_check("Live inference matches saved predictions", "WARN",
                                "Could not find matching UIDs between live run and saved CSV to compare")
            ...
            report.print_check("Live inference matches saved predictions", "FAIL",
                                ...
                                f"Saved predictions may not correspond to this model.")


        # Check live inference produces sensible distribution
        ...
        if live_auc is not None:
            if live_auc >= 0.5:
                ...
                report.print_check("Live inference AUROC on sample", "WARN",
                                    ...
                                    f"(small sample, may be noise — check full test set)")
...
def check_7_label_split_integrity(report):
    """
    Verify the train/val/test split has no patient overlap (data leakage check).
    A patient's patches must appear in exactly one split.
    """
...
def check_8_permutation_baseline(report, pred_df):
    """
    Shuffle the predicted probabilities randomly and recompute AUROC.
    The real model's AUROC must be significantly higher than a shuffled baseline.
    If not, the model result could be due to chance.
    """
...
def check_9_prediction_score_separation(report, pred_df):
    """
    Positive patches should have significantly higher predicted probabilities
    than negative patches. Tests this statistically with Mann-Whitney U test.
    """
...
def main():
    ...
    print("=" * 70)
    print("  MODEL RESULTS AUDIT — DenseNet121 Lung Nodule Classification")
