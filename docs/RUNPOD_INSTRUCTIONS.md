...
The training script has been configured with:
...
- It will save the best model to `outputs/checkpoints/best_model.pth`.
...
1.  **Save Training Logs:**
    *   The training script prints accuracy and loss to the terminal but does not automatically save a log file.
    *   **Action:** Select all text in your RunPod terminal, copy it, and paste it into `outputs/logs/training_logs.txt` (locally). You will need this to plot your training curves later.

2.  **Download the Model:**
    You need to get the checkpoint off the RunPod instance.
    *   **Option A (RunPod Jupyter):** Right-click `outputs/checkpoints/best_model.pth` -> Download.
    *   **Option B (SCP):**
        ```powershell
        scp -P [PORT] root@[IP]:/root/LUNGCANCER3D/outputs/checkpoints/best_model.pth .
        ```
